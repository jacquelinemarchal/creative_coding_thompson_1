...
var recursive_circles = () => { 
    let originalRad = 5;
    // draw first circle
    circle(width/2, height/2, originalRad);
    // call right-circle-drawing function
    drawCircles(width/2+originalRad, originalRad*2, 0);
    // call left-circle-drawing function
    drawCircles(width/2-originalRad, originalRad*2, 1);
}
var drawCircles = (x, rad, dir) => {
    if (x > width || x < 0){
        return -1;
    }
    else{
        ...
        if (dir === 0){ 
            drawCircles(width/2+newRadius, newRadius, 0)
        }
        else{
            drawCircles(width/2-newRadius, newRadius, 1)
        }
    }
}
